import { Text, View, StyleSheet, TextInput, Keyboard, TouchableWithoutFeedback, KeyboardAvoidingView, Alert, TouchableOpacity } from 'react-native';
import {useState, useRef} from 'react'
import axios from "axios";
import * as SecureStore from 'expo-secure-store';

const DreamEntry = ({navigation}) => {
  // used to pass date value as placeholder in the date field for the dream
  ...
  // will navigate to dream vault once the user press enter the vault
  } 
  const personalFavorites = () => {
    navigation.navigate('DreamFavoriteSelection')
  }
  const timeFunction = () => {
    const date = new Date();
    ...
    let currentDate = `Date: ${month}/${day}/${year} Time: ${time}`
    return(currentDate)
  }
  const saveDream = async () => {
    ...
    const postRequest = await axios.post(`http://10.0.0.211:8000/save_user_dream_vault`,{dreamDate:currentDateTime, dreamSubject:subject, dreamNote:dream, profileName:profileName})
    ...
    if (postRequest.data === "Successfully added to vault"){
      ...
      await setSubject()
      await setDream()
      Alert.alert('🔐', 'Dream added to vault')
      // once post request is ran, the state value will also change to empty.
    }
    else{
      Alert.alert('❌', 'Dream not added to vault. Try again.')
          ...
          <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
            {/* this will make the keyboard disappear when user press on screen */}
              ...
              <View style={styles.buttonTopView}>
                <TouchableOpacity onPress={(saveDream)} style={styles.buttonStyle}><Text style={styles.buttonText}>Save to vault 🔐</Text></TouchableOpacity>
              </View>
